...
def get_orphaned_files_generator(
    model, field_name, dirname, extra_filter_kwargs=None
    ):
    ...
    for path, rel_file_path in paths:
        ...
        if not model._default_manager\
            .filter(**extra_filter_kwargs).exists():
...
def delete_orphaned_files(model, field_name, dirname):
    ...
    for rel_file_path, path in get_orphaned_files_generator(
        ...
        if os.path.isfile(path):
            logger.info(
                "deleting orphaned file: %s (field: %s.%s)",
                path, model.__name__, field_name
                )
            os.remove(path)
